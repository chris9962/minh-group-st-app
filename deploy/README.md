...
| `workers.sh` | dựng image worker, thử `--check`, thay hai container `mgst-api-worker` và `mgst-photo-check` với đủ cờ; chạy sau `deploy.sh` khi có sửa ở worker hoặc `src/server` |
