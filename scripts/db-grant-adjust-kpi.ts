...
/**
 * Cấp `*:adjust-kpi` cho các tài khoản TOÀN QUYỀN đã có trong database.
 *
 ...
 * transaction với `ALTER TYPE … ADD VALUE` (migration 0046 thêm `'adjust-kpi'`).
 *
 ...
 * thiếu đúng một dòng — họ vừa không cộng điểm được, vừa rớt khỏi trạng thái
 * toàn quyền ở lưới P-92 mà không ai báo. `db:grant-missing` không lo được vì
 ...
 * khoản toàn quyền do người thật cấp. Tài khoản thường muốn có `adjust-kpi`
 * thì cấp tay ở P-92.
 *
 * Chạy khô để xem trước, không ghi gì:
 *   bun run db:grant-adjust-kpi -- --dry-run
 *
 ...
 *   bun run db:grant-adjust-kpi -- --as=admin
 *
...
async function main() {
  ...
  const people = await db.execute<{ id: string; full_name: string }>(
    sql`select u.id, u.full_name from users u
        where u.active
          ...
          and not exists (
            select 1 from user_permissions p
            where p.user_id = u.id and p.module = '*' and p.action = 'adjust-kpi'
          )
        order by u.full_name`,
  );

  if (people.rows.length === 0) {
    console.log("Mọi tài khoản toàn quyền đang hoạt động đã có `*:adjust-kpi`.");
    await pool.end();
    return;
  }
  ...
  for (const p of people.rows) console.log(`  ${p.full_name} · *:adjust-kpi (company)`);
  ...
  for (const p of people.rows) {
    await db.execute(sql`
      ...
      values (${p.id}, '*'::module_key, 'adjust-kpi'::action_key, 'company'::scope_key)
    ...
    await db.execute(sql`
      ...
      values (
        ...
        ${`db:grant-adjust-kpi cấp *:adjust-kpi (company) cho ${p.full_name}`},
