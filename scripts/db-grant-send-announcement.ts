...
/**
 * Cấp `*:send-announcement` cho các tài khoản TOÀN QUYỀN đã có trong database.
 *
 * `system:send-announcement` mở nút Gửi thông báo chung ở màn Thông báo: gõ
 * tiêu đề và nội dung, bấm một lần là mọi nhân viên đang hoạt động nhận được.
 *
 * ⚠️ Một lượt bấm là hàng trăm dòng thông báo cộng hàng trăm gói tin đẩy, và
 * KHÔNG thu hồi được. Cấp quyền này cho ai thì cân nhắc đúng như vậy.
 *
 * Vì sao là script chứ không phải migration: cùng lý do với
 * `db-grant-handle-feedback.ts` — drizzle bọc cả loạt migration vào MỘT
 * transaction, mà Postgres cấm dùng giá trị enum vừa thêm trong cùng transaction
 * với `ALTER TYPE … ADD VALUE` (migration 0085 thêm `'send-announcement'`).
 *
 ...
 * thiếu đúng một dòng — họ vừa không gửi được thông báo chung, vừa rớt khỏi
 * trạng thái toàn quyền ở lưới P-92 mà không có gì báo. `db:grant-missing`
 * không lo được vì nó cố ý từ chối module `*`.
 *
 ...
 * khoản toàn quyền do người thật cấp. Tính đến 2026-08-30 là 3 tài khoản, gồm
 * Giám đốc — script KHÔNG đếm cứng con số đó, vì số tài khoản toàn quyền đổi
 * được còn điều kiện thì không.
 *
 * Người không thuộc nhóm toàn quyền mà cần gửi thông báo chung thì cấp tay ở P-92.
 *
 * Chạy khô để xem trước, không ghi gì:
 *   bun run db:grant-send-announcement -- --dry-run
 *
 ...
 *   bun run db:grant-send-announcement -- --as=admin
 *
...
async function main() {
  ...
  const people = await db.execute<{ id: string; full_name: string }>(
    sql`select u.id, u.full_name from users u
        where u.active
          ...
          and not exists (
            select 1 from user_permissions p
            where p.user_id = u.id and p.module = '*' and p.action = 'send-announcement'
          )
        order by u.full_name`,
  );

  if (people.rows.length === 0) {
    console.log("Mọi tài khoản toàn quyền đang hoạt động đã có `*:send-announcement`.");
    await pool.end();
    return;
  }
  ...
  for (const p of people.rows) console.log(`  ${p.full_name} · *:send-announcement (company)`);
  ...
  for (const p of people.rows) {
    await db.execute(sql`
      ...
      values (${p.id}, '*'::module_key, 'send-announcement'::action_key, 'company'::scope_key)
    ...
    await db.execute(sql`
      ...
      values (
        ...
        ${`db:grant-send-announcement cấp *:send-announcement (company) cho ${p.full_name}`},
