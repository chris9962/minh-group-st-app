...
/**
 * Cấp `*:handle-feedback` cho các tài khoản TOÀN QUYỀN đã có trong database.
 *
 * `system:handle-feedback` mở hộp góp ý P-96: đọc mọi góp ý và đánh dấu đã xử
 * lý. Nút GỬI góp ý thì không gác quyền nào — xem migration 0052.
 *
 * Vì sao là script chứ không phải migration: cùng lý do với
 * `db-grant-set-status.ts` — drizzle bọc cả loạt migration vào MỘT transaction,
 * mà Postgres cấm dùng giá trị enum vừa thêm trong cùng transaction với
 * `ALTER TYPE … ADD VALUE` (migration 0052 thêm `'handle-feedback'`).
 *
 ...
 * thiếu đúng một dòng — họ vừa không mở được hộp góp ý, vừa rớt khỏi trạng thái
 * toàn quyền ở lưới P-92 mà không có gì báo. `db:grant-missing` không lo được
 * vì nó cố ý từ chối module `*`.
 *
 ...
 *
 * Người không thuộc nhóm toàn quyền mà cần xử lý góp ý thì cấp tay ở P-92.
 *
 * Chạy khô để xem trước, không ghi gì:
 *   bun run db:grant-handle-feedback -- --dry-run
 *
 ...
 *   bun run db:grant-handle-feedback -- --as=admin
 *
...
async function main() {
  ...
  const people = await db.execute<{ id: string; full_name: string }>(
    sql`select u.id, u.full_name from users u
        where u.active
          ...
          and not exists (
            select 1 from user_permissions p
            where p.user_id = u.id and p.module = '*' and p.action = 'handle-feedback'
          )
        order by u.full_name`,
  );

  if (people.rows.length === 0) {
    console.log("Mọi tài khoản toàn quyền đang hoạt động đã có `*:handle-feedback`.");
    await pool.end();
    return;
  }
  ...
  for (const p of people.rows) console.log(`  ${p.full_name} · *:handle-feedback (company)`);
  ...
  for (const p of people.rows) {
    await db.execute(sql`
      ...
      values (${p.id}, '*'::module_key, 'handle-feedback'::action_key, 'company'::scope_key)
    ...
    await db.execute(sql`
      ...
      values (
        ...
        ${`db:grant-handle-feedback cấp *:handle-feedback (company) cho ${p.full_name}`},
