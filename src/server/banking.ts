...
async function bankIdsOf(code: string): Promise<string[] | null> {
  // NHIỀU mã ngăn bằng dấu phẩy — màn Xuất dữ liệu gửi những ngân hàng người
  // dùng tick ở ô lọc. Mã ngân hàng không chứa dấu phẩy nên tách thẳng được;
  // một mã lẻ vẫn đi qua đây không đổi hành vi.
